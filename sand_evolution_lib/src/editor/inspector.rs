use crate::ecs::components::{Name, Position, Rotation, Scale, Script};
...
impl EditorInspector {
    ...
    fn show_multiple_entities(ui: &mut Ui, entities: &[Entity], world: &mut World) {
        ...
        ui.collapsing("Transform (Multi-edit)", |ui| {
            ...
            let all_have_position = entities.iter().all(|e| positions.get(*e).is_some());
            ...
            let rotations_vec: Vec<_> = entities.iter().filter_map(|e| rotations.get(*e)).collect();
            ...
            let scales_vec: Vec<_> = entities.iter().filter_map(|e| scales.get(*e)).collect();

            if !scales_vec.is_empty() {
                let first_scale = scales_vec[0];
                let all_same = scales_vec
                    .iter()
                    .all(|s| s.x == first_scale.x && s.y == first_scale.y);
