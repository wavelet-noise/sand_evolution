...
impl EditorToolbar {
    pub fn ui(ui: &mut egui::Ui, editor_state: &mut EditorState, undo_redo: &UndoRedo) {
        ui.horizontal(|ui| {
            ...
            if ui.add_enabled(undo_enabled, Button::new(RichText::new(&undo_text).size(12.0))).clicked() {
            ...
            if ui.add_enabled(redo_enabled, Button::new(RichText::new(&redo_text).size(12.0))).clicked() {
                // Will be handled by keyboard handler
            }
            ...
            if select_selected {
                select_btn = select_btn.fill(egui::Color32::from_rgba_unmultiplied(100, 150, 255, 100));
            }
            if ui.add(select_btn).on_hover_text("Select tool (Q)").clicked() {
            ...
            if rotate_selected {
                rotate_btn = rotate_btn.fill(egui::Color32::from_rgba_unmultiplied(100, 150, 255, 100));
            }
            if ui.add(rotate_btn).on_hover_text("Rotate tool (R)").clicked() {
            ...
            if scale_selected {
                scale_btn = scale_btn.fill(egui::Color32::from_rgba_unmultiplied(100, 150, 255, 100));
            }
