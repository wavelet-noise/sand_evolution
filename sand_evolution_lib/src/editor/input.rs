                    ...
                    if !editor_state.selected_entities.is_empty() {
                        {
                            ...
                            for entity in &editor_state.selected_entities {
                                if let Some(pos) = positions.get(*entity) {
                                    ...
                                    if editor_state.gizmo_handle
                                        != crate::editor::state::GizmoHandle::None
                                    {
                                        break;
                                    }
                                }
                    ...
                    if editor_state.gizmo_handle == crate::editor::state::GizmoHandle::None {
                        match editor_state.mode {
                            EditorMode::Select => {
                                ...
                                editor_state.selection_rect =
                                    Some((world_pos.x, world_pos.y, world_pos.x, world_pos.y));
                            }
                            _ => {}
                        }
                    }
                ...
                EditorMode::Move => {
                    if let Some(pos) = positions.get_mut(*entity) {
                        let (snapped_x, snapped_y) =
                            editor_state.snap_position(pos.x + delta_x, pos.y + delta_y);
                        pos.x = snapped_x;
                        pos.y = snapped_y;
                    }
                }
